                      ...
                      FloatingActionButton(
                        onPressed: ()async {
                          if ( widget.validate != null) {
                            if (await widget.validate(_city)) {
  ...
  Future<Map<String, String>> getAddress(LatLng location) async {
    try {
      ...
      if(results != null && results.isNotEmpty){
        ...
        List addressComponent = result["address_components"];
        if(addressComponent != null && addressComponent.isNotEmpty){
          var addressLevel = addressComponent.firstWhere((element) => element["types"].contains("administrative_area_level_1"),orElse: ()=> null);
          if(addressLevel == null)
            addressLevel = addressComponent.firstWhere((element) => element["types"].contains("administrative_area_level_2"),orElse: ()=> null);
          if(addressLevel!= null)
          city = addressLevel["short_name"];
        }
      }
...
class _MapFabs extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      margin: const EdgeInsets.only(bottom: 68, right: 16),
      child: Column(
